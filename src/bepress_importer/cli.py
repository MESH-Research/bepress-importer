...
def convert(
    ...
    Writes one JSON file per matched sheet (each targets one KC Works
    collection) plus a report.json of conversion issues.
    """
    workbook = read_workbook(input_file)
    if sheets:
...
def fix(data_dir, log_path, output_dir, rule_names, record_id, yes, as_of) -> None:
    """Interactively accept or reject proposed fixes.

    Accepted changes are appended to the wrangling log and the wrangled
    output directory is regenerated by replay.
    """
...
def apply(data_dir, log_path, output_dir) -> None:
    """Replay the wrangling log over converter output.

    Regenerates the wrangled output directory; exits 1 if any logged
    change no longer matches the data (conflict).
    """
...
def upload(metadata_file, collection_id, api_url, api_key_env, api_key_file, receipts_path,
    ...
    """Upload a finished collection JSON file to KC Works.

    POSTs the records to /api/import/<collection-id> and writes per-record
    receipts mapping each import-recid to its created record URL.
    """
    import json as json_module
    import os
